...
-- شغّل الملف ده مرة واحدة على قاعدة البيانات (Supabase SQL Editor) قبل
-- استيراد وتفعيل n8n/ai-chat-assistant.json
-- ============================================================================

-- ----------------------------------------------------------------------------
-- 0) إصلاح إضافي مهم (وُجد أثناء تحليل الكود):
-- عمود current_surah متوقّع فعليًا في الكود الحالي (js/students.js لما بتحفظ
-- بيانات الطالب، js/student.js لما بتعرضها، js/dashboard.js في الملخص
-- اليومي) لكنه مش موجود في قاعدة البيانات الفعلية حسب الـ schema اللي بعتها.
...
-- (آخر عدد معيّن من الرسائل بيتجاب من هنا مع كل طلب جديد، راجع الشرح
